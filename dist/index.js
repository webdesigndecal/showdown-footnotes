  ...
  return [{
    ...
    filter: function filter(text) {
      return text.replace(/^\[\^([\d\w]+)\]:( |\n)((.+\n)*.+)$/mg, function (str, name, _, content) {
        return '<small class="footnote" id="footnote-' + name + '"><a href="#footnote-' + name + '-backlink"><sup>[' + name + ']</sup></a>: ' + content + '</small>';
      });
    }
  }, {
    type: 'lang',
    filter: function filter(text) {
      return text.replace(/\[\^([\d\w]+)\]/mg, function (str, name) {
        return '<a href="#footnote-' + name + '" id="footnote-' + name + '-backlink"><sup>[' + name + ']</sup></a>';
      });
    }
  }];
};